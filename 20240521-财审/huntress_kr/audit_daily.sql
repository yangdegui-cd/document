create table if not exists hive.huntress_kr_w.dws_audit_daily_di
(date date,
role_id varchar,
channel varchar,
money bigint,
...
delete from hive.huntress_kr_w.dws_audit_daily_di where part_date >= '{yesterday}' and part_date <= '{today}';

insert into hive.huntress_kr_w.dws_audit_daily_di
...
(case when date = b.firstpay_date then coalesce(core_remain, 0) + coalesce(core_gain, 0) + coalesce(money, 0) * 0.05
else coalesce(core_gain, 0) + coalesce(money, 0) * 0.05 end) as core_gain_addmoney, 
 -- 用户付费首日，充值前一日的剩余计算为第一条新增日志; 充值金额按照汇率增加
(coalesce(core_cost, 0) + coalesce(money, 0) * 0.05) as core_cost_addmoney, 
...
part_date
from hive.huntress_kr_r.dwd_gserver_diamondcost_live
where part_date >= '{yesterday}' and part_date <= '{today}'
...
part_date
from hive.huntress_kr_r.dwd_gserver_additem_live a
left join hive.huntress_kr_w.dim_gserver_audit_item b
...
b.item_type, coalesce(b.item_price, 0) as item_price, b.item_name,
part_date
from hive.huntress_kr_r.dwd_gserver_costitem_live a
left join hive.huntress_kr_w.dim_gserver_audit_item b
