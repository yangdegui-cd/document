...
###  
create table if not exists hive.huntress_jp_w.dws_audit_daily_di
(date date,
role_id varchar,
channel varchar,
money bigint,
...
delete from hive.huntress_jp_w.dws_audit_daily_di where part_date >= {beginning_of_month} and part_date <= {end_of_month};

insert into hive.huntress_jp_w.dws_audit_daily_di
...
where part_date >= $start_date and part_date <= {end_of_month}
),

user_info as
(select role_id, channel, install_date, firstpay_date
from hive.huntress_jp_w.dws_user_info_di a
...
(case when date = b.firstpay_date then coalesce(core_remain, 0) + coalesce(core_gain, 0) + coalesce(money, 0) * 0.5 
else coalesce(core_gain, 0) + coalesce(money, 0) * 0.5 end) as core_gain_addmoney,  -- 用户付费首日，充值前一日的剩余计算为第一条新增日志; 充值金额按照汇率增加
(coalesce(core_cost, 0) + coalesce(money, 0) * 0.5) as core_cost_addmoney,  -- 充值金额按照汇率增加，当前按照0.5计算
core_end,
...
part_date
from hive.huntress_jp_r.dwd_gserver_diamondcost_live
where part_date >= $start_date and part_date <= {end_of_month}
and diamond_cost_reason not in (2, 19, 32, 33, 68, 76, 81, 85, 88, 99, 125, 141, 316, 317, 318, 319, 320)  -- 2在消耗中不统计
),
...
part_date
from hive.huntress_jp_r.dwd_gserver_additem_live a
left join hive.huntress_jp_w.dim_gserver_audit_item b
...
b.item_type, coalesce(b.item_price, 0) as item_price, b.item_name,
part_date
from hive.huntress_jp_r.dwd_gserver_costitem_live a
left join hive.huntress_jp_w.dim_gserver_audit_item b
