...
host=119.28.41.240
port=22
desdir=/data1/warship/jp/warship_jp_payment_app
user='warship'
...
#1.压缩
mysql -h10.0.18.20 -uworldship -pworldship -Dworldship -N  -e "select 'payment',billno,openid,region,uid,channel,device,account,money,REPLACE(orderid, ',', ''),level,viplevel,status,time,payway,origin from pay where time>='${day} 00:00:00' and time<= '${day} 23:59:59'" -B -s | sed 's/\t/,/g' > ${tempdir}${todata}
