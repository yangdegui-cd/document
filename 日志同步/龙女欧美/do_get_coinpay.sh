...
port=22
user=huntresshq
desdir=/data1/huntress/oumei/huntress_oumei_payment_web
password=aRVv!9ED

logdir=/data/stat/Log*/

tempdir=/data/tmpdir/pay_list/
todata=coinpaylist_${day}.data
echo 'start upload data:'${day}
#1.压缩
mysql -h10.200.16.16 -udragon -pdragongirl -N  -e "select  dragongirl.coin_pay.*  from dragongirl.coin_pay where time>='${day} 00:00:00' and time<= '${day} 23:59:59'" -B -s | sed 's/\t/,/g' > ${tempdir}${todata}
