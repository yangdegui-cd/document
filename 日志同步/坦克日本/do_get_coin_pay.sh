...
port=22
user=tank
desdir=/data/tank/japan/payment
password=TaNk123
...
#1.压缩
mysql -h10.0.0.17 -utank -ptank@knat -N  -e "select 'payment', tank.coin_pay.*  from tank.coin_pay where time>='${day} 00:00:00' and time<= '${day} 23:59:59' and status = 0" -B -s | sed 's/\t/,/g' > ${tempdir}${todata}
