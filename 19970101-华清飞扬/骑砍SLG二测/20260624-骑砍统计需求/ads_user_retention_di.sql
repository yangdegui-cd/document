...
(date, install_date, zone_id, channel, is_old_user, os, break_type, retention_day, 
...
currency, money, online_time, is_old_user
from hive.qkslg2t_om_w.dws_user_daily_di 
), 

user_daily_join as(
select a.date, a.part_date, a.role_id, a.is_old_user,
...
retention_info as(
select date, install_date, zone_id, channel, is_old_user, os, break_type, retention_day,
...
retention_all as(
select install_date, zone_id, channel, is_old_user, os, break_type, 
...
data_cube as(
select distinct install_date, zone_id, channel, is_old_user, os, break_type, t.retention_day
from retention_info
...
a.install_date, a.zone_id, a.channel, a.is_old_user, a.os, a.break_type, a.retention_day,
...
select date, install_date, zone_id, channel, is_old_user, os, break_type, retention_day, 
active_users, pay_users, newpay_users, money, money_rmb, online_time, 
sum(newpay_users) over (partition by install_date, zone_id, channel, is_old_user , break_type, os order by retention_day
rows between unbounded preceding and current row) as payuser_ac,
sum(money) over (partition by install_date, zone_id, channel, is_old_user, break_type, os order by retention_day
rows between unbounded preceding and current row) as money_ac,
sum(money_rmb) over (partition by install_date, zone_id, channel, is_old_user, break_type, os order by retention_day
