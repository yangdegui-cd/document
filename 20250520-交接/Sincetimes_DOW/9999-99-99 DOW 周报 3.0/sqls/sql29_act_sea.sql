...
* @Last Modified time: 2025-02-24 13:56:47
*/
...
activitygetreward_select as(
select part_date, date(part_date) as date, d.act_tag, 
'档位奖励领取' as type, coalesce(b.pay_tag, '未活跃') as pay_tag, 
a.role_id, a.content_id as dis_type1, null as dis_type2, null as target_sum
from hive.dow_jpnew_r.dwd_gserver_activitygetreward_live a
left join month_tag b
on a.role_id = b.role_id
left join act_tag d
on date(a.part_date) = d.date
...
left join act_tag d
on date(a.part_date) = d.date
where a.payment_item_id in ('gold_305', 'gold_306', 'gold_307')
and b.is_test is null
), 
...
union all
select * from activitygetreward_select)
