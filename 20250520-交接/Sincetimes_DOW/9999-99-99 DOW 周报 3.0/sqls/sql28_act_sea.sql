...
* @Last Modified time: 2025-02-24 13:46:53
*/
...
sum(case when payment_item_id in('gold_305', 'gold_306', 'gold_307') then coalesce(rawmoney, 0) * 0.052102 + coalesce(sincetime_money, 0) else null end) as act_rmb
...
item_agg as(
select part_date, date(part_date) as date, role_id, 
sum(case when event_type = 'cost' and item_id = '10999' then item_num else null end) as item_cost, 
sum(case when event_type = 'gain' and item_id = '10999' then item_num else null end) as item_add, 
sum(case when event_type = 'cost' and item_id = '360047' then item_num else null end) as exchange_cost, 
sum(case when event_type = 'gain' and item_id = '360047' then item_num else null end) as exchange_add,  
sum(case when event_type = 'cost' and item_id = '11000' then item_num else null end) as summon_count
from hive.dow_jpnew_r.dwd_gserver_itemchange_live
where part_date >= date_format(date_add('month', -6, current_date), '%Y-%m-%d')
and item_id in ('10999', '11000', '360047')
group by 1, 2, 3
), 
...
b.act_rmb, c.item_cost, c.item_add, 
c.exchange_cost, c.exchange_add, c.summon_count, 
...
a.core_add, a.core_cost, a.core_end, 
a.sincetimes_add, a.sincetimes_cost, a.sincetimes_end, 
a.act_rmb, a.item_cost, a.item_add, 
a.exchange_cost, a.exchange_add, a.summon_count, 
