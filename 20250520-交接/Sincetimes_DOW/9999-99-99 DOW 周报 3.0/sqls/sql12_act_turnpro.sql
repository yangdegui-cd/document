...
* @Last Modified time: 2025-02-17 11:42:02
*/
...
and payment_item_id in ('gold_60006', 'gold_60007', 'gold_60008', 'gold_60009', 'gold_60010')
group by 1, 2, 3
), 

item_agg as(
select part_date, date(part_date) as date, role_id, 
sum(case when event_type = 'cost' and item_id = '11490' then item_num else null end) as summon_itemcost, 
sum(case when event_type = 'gain' and item_id = '11490' then item_num else null end) as summon_itemadd, 
sum(case when event_type = 'cost' and item_id = '11491' then item_num else null end) as exchange_itemcost, 
sum(case when event_type = 'gain' and item_id = '11491' then item_num else null end) as exchange_itemadd
from hive.dow_jpnew_r.dwd_gserver_itemchange_live
where part_date >= date_format(date_add('month', -6, current_date), '%Y-%m-%d')
and item_id in ('11490', '11491')
group by 1, 2, 3
), 
...
b.act_rmb, c.summon_itemcost, c.summon_itemadd, c.summon_itemcost/2 as summon_count, 
c.exchange_itemcost, c.exchange_itemadd, 
...
a.exchange_itemcost, a.exchange_itemadd, 
