...
* @Last Modified time: 2025-02-17 11:42:38
*/
...
select part_date, date(part_date) as date, d.act_tag, 
'兑换情况' as type, coalesce(b.pay_tag, '未活跃') as pay_tag, 
...
left join act_tag d
on date(a.part_date) = d.date
where a.payment_item_id in ('gold_60006', 'gold_60007', 'gold_60008', 'gold_60009', 'gold_60010')
and b.is_test is null
), 
...
chipslotget_select as(
select part_date, date(part_date) as date, d.act_tag, 
'概率' as type, coalesce(b.pay_tag, '未活跃') as pay_tag, 
a.role_id, cast(probability1+probability2 as varchar) as dis_type1, null as dis_type2, item_num as target_sum
from hive.dow_jpnew_r.dwd_gserver_chipslotsget_live a
left join month_tag b
on a.role_id = b.role_id
left join act_tag d
on date(a.part_date) = d.date
...
union all
select * from chipslotget_select)
