...
* @Last Modified time: 2025-02-17 11:43:11
*/
...
where c.payment_act in ('珍珠', '网页充值')
...
select part_date, date(part_date) as date, d.act_tag, 
coalesce(b.pay_tag, '未活跃') as pay_tag, a.role_id, 
a.payment_itemid, c.payment_act, c.payment_name, 
...
where c.payment_act in ('珍珠', '网页充值')
and b.is_test is null
and a.pay_source = 'web'
), 

payment_union_log as(
select * from paymentdetail_log
union all
