...
* @Last Modified time: 2025-03-03 11:50:22
*/
...
item_log as(
select part_date, date(part_date) as date, event_time,
d.act_tag, a.role_id, event_type, reason, item_id, item_num
...
sum(item_num/2) as summon_count
from item_log 
...
item_agg as(
select act_tag, role_id, 
(case when cast(item_id as bigint) >= 95000 and cast(item_id as bigint) <= 95483 then '90006' else item_id end) as itemid_fit, 
sum(item_num) as item_count
from item_log
where reason = '910'
and event_type = 'gain'
group by 1, 2, 3
...
from hive.dow_jpnew_r.dwd_gserver_chipslotschoice_live a
left join month_tag b
on a.role_id = b.role_id
left join act_tag d
on date(a.part_date) = d.date
