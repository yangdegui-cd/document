...
(case when 'warship_jpnew' in ('tank_cnnew') and substr(a.currency, 1, 3) = 'CNY' then money
when 'warship_jpnew' in ('warship_cnnew', 'warship2_cnnew', 'modernship_cnnew') and substr(a.currency, 1, 3) = 'CNY' then money * 0.01
when 'warship_jpnew' in ('huntress_jpnew', 'huntress_krnew', 'huntress_twnew', 'warship_jpnew', 'tank_jpnew')  then money * b.exchange_rate
when 'warship_jpnew' in ('huntress_omnew', 'dow_jpnew', 'warship2_asianew') then money * b.exchange_rate * 0.01
else money * b.exchange_rate end) as money_rmb,
online_time
from hive.warship_jpnew_r.dwd_merge_base_live a
