WITH AdjustCost AS
     ...
     UserInstallTag AS
         (SELECT "#user_id",
          ...
          FROM ta.user_result_cluster_99
          GROUP BY 1),
...
-- 实际花费总和
-- select sum(CAST(cost AS DECIMAL(10, 2))) cost  FROM ta.v_event_99 WHERE "$part_event" = 'adjust_cost' AND "$part_date" >= '2023-01-01'
...
-- select ResultTable.*
-- from ResultTable
--          left join (select * from ta.v_event_99 where "$part_event" = 'adjust_cost_detail') e
--                    on ResultTable."#event_id" = e."#event_id"
-- where e.ad_cost != ResultTable.ad_cost or e.other_network_ad_cost != ResultTable.other_network_ad_cost
