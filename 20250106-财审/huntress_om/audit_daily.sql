...
###  
create table if not exists hive.huntress_twnew_w.dws_audit_daily_di
(date date,
role_id varchar,
channel varchar,
money bigint,
...
delete from hive.huntress_twnew_w.dws_audit_daily_di where part_date >= '{beginning_of_month}' and part_date <= '{end_of_month}';

insert into hive.huntress_twnew_w.dws_audit_daily_di
...
(case when date = b.firstpay_date then coalesce(core_remain, 0) + coalesce(core_add, 0) + coalesce(money, 0) * 0.6
else coalesce(core_add, 0) + coalesce(money, 0) * 0.6 end) as core_add_addmoney, 
(coalesce(core_cost, 0) + coalesce(money, 0) * 0.6) as core_cost_addmoney, 
core_end,
...
part_date
from hive.huntress_twnew_r.dwd_gserver_corechange_live
where part_date >= '{beginning_of_month}' and part_date <= '{end_of_month}'
and event_type = 'cost'
...
part_date
from hive.huntress_twnew_r.dwd_gserver_itemchange_live a
left join hive.huntress_twnew_w.dim_gserver_audit_item b
