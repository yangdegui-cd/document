...
###  
create table if not exists hive.huntress_jpnew_w.dws_audit_daily_di
(date date,
role_id varchar,
channel varchar,
money bigint,
...
delete from hive.huntress_jpnew_w.dws_audit_daily_di where part_date >= '{beginning_of_month}' and part_date <= '{end_of_month}';

insert into hive.huntress_jpnew_w.dws_audit_daily_di
...
(case when date = b.firstpay_date then coalesce(core_remain, 0) + coalesce(core_add, 0) + coalesce(money, 0) * 0.5 
else coalesce(core_add, 0) + coalesce(money, 0) * 0.5 end) as core_add_addmoney, 
(coalesce(core_cost, 0) + coalesce(money, 0) * 0.5) as core_cost_addmoney, 
core_end,
...
role_id,
diamond_type as cost_type,
reason, 
coalesce(free_num, 0) +  coalesce(paid_num, 0) as core_num,
part_date
from hive.huntress_jpnew_r.dwd_gserver_corechange_live
where part_date >= '{beginning_of_month}' and part_date <= '{end_of_month}'
and event_type = 'cost'
and cast(reason as int) not in (2, 19, 32, 33, 68, 76, 81, 85, 88, 99, 125, 141, 316, 317, 318, 319, 320) 
),
...
a.item_id,
reason, 
item_num, diamond_cost as core_cost, 
b.item_type, coalesce(b.item_price, 0) as item_price, b.item_name,
part_date
from hive.huntress_jpnew_r.dwd_gserver_itemchange_live a
left join hive.huntress_jpnew_w.dim_gserver_audit_item b
