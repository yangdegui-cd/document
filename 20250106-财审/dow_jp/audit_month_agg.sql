...
SELECT 
    ...
    SUM(core_add) - SUM(diamond_cost_once) - SUM(diamond_cost_forever) - SUM(diamond_cost_play), 
    CONCAT(CAST(ROUND(((SUM(core_add) - SUM(diamond_cost_once) - SUM(diamond_cost_forever) - SUM(diamond_cost_play)) / SUM(core_cost)) * 100, 4) AS VARCHAR), '%')
FROM user_daily
GROUP BY 1, 2;
