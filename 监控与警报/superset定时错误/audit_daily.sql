...
###  
create table if not exists hive.huntress_om_w.dws_audit_daily_di
(date date,
role_id varchar,
channel varchar,
money bigint,
...
delete from hive.huntress_om_w.dws_audit_daily_di where part_date >= '{yesterday}' and part_date <= '{today}';

insert into hive.huntress_om_w.dws_audit_daily_di
...
(case when date = b.firstpay_date then coalesce(core_remain, 0) + coalesce(core_gain, 0) + coalesce(money, 0) * 0.6
else coalesce(core_gain, 0) + coalesce(money, 0) * 0.6 end) as core_gain_addmoney, 
...
part_date
from hive.huntress_om_r.dwd_gserver_diamondcost_live
where part_date >= '{yesterday}' and part_date <= '{today}'
...
part_date
from hive.huntress_om_r.dwd_gserver_additem_live a
left join hive.huntress_om_w.dim_gserver_audit_item b
...
b.item_type, coalesce(b.item_price, 0) as item_price, b.item_name,
part_date
from hive.huntress_om_r.dwd_gserver_costitem_live a
left join hive.huntress_om_w.dim_gserver_audit_item b
